...
## Веб-визуализация (React Dashboard)

Можно смотреть результаты не в терминале, а в браузере.

1. Соберите host-версию:

```bash
make host-build
```

2. Поднимите локальный сервер из корня проекта:
...
В `web/` есть React-страница, которая:
- показывает summary (average/best speedup),
- рисует красивую таблицу,
- поддерживает загрузку любого JSON-отчета через кнопку `Upload JSON report`,
- поддерживает кнопку `Run benchmark`, которая запускает `./go` и обновляет таблицу.

Если хотите просто вручную экспортировать JSON без UI-кнопки:
...
Поддерживаются:
- `--json <path>`: сохранить результат в JSON,
- `--no-terminal`: не печатать таблицу в консоль,
